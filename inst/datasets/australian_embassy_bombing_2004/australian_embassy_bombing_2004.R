...
edges <- edges %>%
  ...
  dplyr::rename(from = ID, to = Tie_ID, time = key, edge_strength = value) %>%
  dplyr::mutate(from_class = "person",
         to_class   = "person",
         edge_class = dplyr::case_when(
           edge_strength == 0 ~ "No relationship known",
           edge_strength == 1 ~ "Acquaintances/Distant familiy ties",
           edge_strength == 2 ~ "Friends/Moderately close family ties",
           edge_strength == 3 ~ "Close Friends/Family"),
