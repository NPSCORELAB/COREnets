...
nodes <- .corenets_read_csv(
  .corenets_sys_file("datasets/christmas_eve_bombings_2000/CE_Nodes_Public_Version2.csv")
)
edges <- .corenets_read_csv(
  .corenets_sys_file("datasets/christmas_eve_bombings_2000/CE_Relations_Public_Version2.csv")
)

# Clean up the edges data ======================================================
edges <- edges %>%
  dplyr::select(-c(Operation_ID, Op_role, Tie_Year, Tie_Extinguish)) %>%
...
.description <- .corenets_read_lines(
  .corenets_sys_file("datasets/christmas_eve_bombings_2000/description.txt")
)

.abstract <- .corenets_read_lines(
  .corenets_sys_file("datasets/christmas_eve_bombings_2000/abstract.txt")
)

.bibtex <- bibtex::read.bib(
  .corenets_sys_file("datasets/christmas_eve_bombings_2000/refs.bib")
)
...
.reference <- list(
  ...
  name         = "christmas_eve_bombings_2000",
  tags         = c("terrorism"),
...
christmas_eve_bombings_2000 <- list(
  reference = .reference,
  network  = .network
)

christmas_eve_bombings_2000
