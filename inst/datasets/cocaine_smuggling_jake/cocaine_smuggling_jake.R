...
# read edges data ==============================================================
edges <- COREnets:::read_matrix("inst/datasets/cocaine_smuggling_jake/COCAINE_JAKE.csv") %>%
...
# build final dataset ==========================================================
.description <- readLines("inst/datasets/cocaine_smuggling_jake/description.txt",
                          warn = FALSE)

.abstract <- readLines("inst/datasets/cocaine_smuggling_jake/abstract.txt",
                       warn = FALSE)

.bibtex <- bibtex::read.bib("inst/datasets/cocaine_smuggling_jake/refs.bib")
...
.codebook <- data.frame(
  ...
  is_dynamic  = c(FALSE),
  is_weighted = c(FALSE),
  definition  = c("Undirected valued relationship for actors."),
  stringsAsFactors = FALSE
)

.reference <- list(
  title        = "Cocaine Smuggling Operation JAKE",
  name         = "cocaine_smuggling_jake",
...
cocaine_smuggling_jake <- list(
  reference = .reference,
  network   = .network
)

cocaine_smuggling_jake
