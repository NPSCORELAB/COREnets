...
edges <- COREnets:::read_matrix(
  .corenets_sys_file("datasets/harry_potter_death_eaters/Death_Eaters_Snape.csv")
  ) %>%
...
.description <- .corenets_read_file(
  .corenets_sys_file("datasets/harry_potter_death_eaters/description.txt")
)

.abstract <- .corenets_read_file(
  .corenets_sys_file("datasets/harry_potter_death_eaters/abstract.txt")
)

.bibtex <- bibtex::read.bib(
  .corenets_sys_file("datasets/harry_potter_death_eaters/refs.bib")
)
...
.network <- list(
  metadata   = COREnets:::unnest_edge_class(
    g = g, edge_class_name = "edge_class"
  ) %>%
