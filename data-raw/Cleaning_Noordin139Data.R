...
edges <- files %>%
  ...
  purrr::imap_dfr(
    ~ readxl::read_excel(path = files, sheet = .x) %>%
      ...
      COREnets::to_matrix() %>%
      COREnets::to_graph() %>%
      # COREnets::to_one_mode(project = "rows") %>%
...
nodes <- nodes %>%
  filter(name != "Suramto (Deni)" & name != "Mohammed Faiz") %>%
  mutate(
    # Education Level: =========================================================
    # Defined as highest degree attainedm level taught at, studied, participated
    # in, or attended.
    education_level = `Education Level`,
    hr_education_level = case_when(`Education Level` == 0 ~ "Unknown",
    ...
    hr_contact_w_people = case_when(`Contact with People` ==  0 ~ "Unknown",
    ...
    hr_military_training = case_when(`Military Training`  ==  0 ~ "Unknown",
    ...
    hr_nationality = case_when(Nationality == 1 ~ "Afghanistan",
    ...
    hr_current_status_per_icg_article = case_when(`Current Status (ICG Article)` == 0 ~ "Dead",
    ...
    hr_current_status_updated = case_when(`Current Status (Updated)` == 0 ~ "Dead",
    ...
    hr_role_original_coding = case_when(`Role (Original Coding)` ==  0 ~ "No Info/Unclear",
    ...
    hr_role_expanded_primary = case_when(`Role Expanded (Primary)` ==  1 ~ "Bomb Maker",
    ...
    hr_role_expanded_secondary = case_when(`Role Expanded (Secondary)` ==  1 ~ "Bomb Maker",
    ...
    hr_logistic_function = case_when(`Logistic Function` ==  1 ~ "Providing a safe house",
    ...
    hr_technical_skills = case_when(`Technical Skills` == 1 ~ "Bomb-making",
    ...
    hr_primary_group_affiliation = case_when(`Primary Group Affiliation` == 0 ~ "None (Noordin)",
...
.network <- list(
  metadata = list(
    ...
    node_type    = c("people", "organization", "events"),
...
.introduction <- "These data were drawn primarily from 'Terrorism in Indonesia: 
Noordin's Networks,' a publication of the International Crisis Group (2006) and 
include relational data on the 79 individuals listed in Appendix C of that
publication. "

.abstract <- "These data were drawn primarily from two International Crisis 
Group (ICG) reports: “Terrorism in Indonesia: Noordin’s Networks” (International
Crisis Group, 2006) and “Indonesia: Noordin Top’s Support Base” (International 
Crisis Group, 2009). The two ICG reports on Noordin contain rich one- and 
two-mode data on a variety of relations and affiliations (friendship, kinship, 
meetings, etc.) along with significant attribute data (education, group 
membership, physical status, etc.). The data were initially structured and 
analyzed by Naval Postgraduate School students in the course “Tracking and 
Disrupting Dark Networks” under the direction of Professor Sean Everton, 
Co-Director of the CORE Lab and Professor Nancy Roberts and then expanded and 
cleaned by Associate Faculty for Instruction, Dan Cunningham. 

Permission is granted to use the data set only for the purpose of internal 
analysis and not for external distribution. "

.codebook <- data.frame(
    relationship = c("Organizations",
                     "Education (Schools)",
                     "Classmates",
                     "Communication Ties",
                     "Kinship Ties",
...
usethis::use_data(noordin_top, overwrite = TRUE)
