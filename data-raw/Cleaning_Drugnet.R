...
nodes <- nodes %>%
  mutate(hr_gender = case_when(Gender == 1 ~ "Male",
                            Gender == 2 ~ "Female",
                            Gender == 3 ~ "Unknown"),
         hr_ethnicity = case_when(Ethnicity == 1 ~ "White/Other",
                               ...
                               Ethnicity == 6 ~ "White/Other",),
         hr_has_tie = ifelse(HasTie==1, TRUE, FALSE)) %>%
  select(name, Gender, Ethnicity, HasTie, hr_ethnicity, hr_gender, hr_has_tie)
...
.introduction <- "These data represent a network of drug users in Hartford.  Ties are directed and represent acquaintanceship. The network is a result of two years of ethnographic observations of people's drug habits. "
...
.codebook <- data.frame(
  relationship = c("Needle Sharing"),
  data_type = c("one-mode"),
  definition = c("Ties are directed and represent the lenidn of drug needles."),
